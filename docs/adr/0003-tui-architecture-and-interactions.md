...
- Actions target checked rows on the current page, falling back to that page's cursor only when none are checked. They never borrow hidden selections from another page. View/filter changes reset transient navigation as defined by the current guide; every action modal states target count and names.
